...
function draw(params={}){
  // 1 - populate the audioData array with the frequency data from the analyserNode
	...
	if(params.showBars){
        ...
        let barHeight = 200;
        let topSpacing = 100;
        let image = document.querySelector("#mag-glass");
	...
	if(params.showCircles){
        ...
        for(let i = 0; i < audioData.length; i++){
            ...
            ctx.arc(canvasWidth/2, canvasHeight/2, circleRadius,2* Math.PI,false);
            ctx.fill();
            ctx.closePath();

            ctx.beginPath();
            ctx.fillStyle = utils.makeColor(0,0,255,.1 - percent/10);
            ctx.arc(canvasWidth/2, canvasHeight/2, circleRadius * 1.5,2* Math.PI,false);
            ctx.fill();
            ctx.closePath();

            ctx.beginPath();
            ctx.fillStyle = utils.makeColor(200,200,0,.5 - percent/5);
            ctx.arc(canvasWidth/2, canvasHeight/2, circleRadius * .5,2* Math.PI,false);
            ctx.fill();
            ctx.closePath();
            ctx.restore();
        }
    ...
    if(params.showLargeSprite){
        ...
        let circleRadius = percent * maxRadius;
        const imageEl = document.querySelector("#mag-glass");
        ...
        console.log(imageEl.width,imageEl.height);
        ctx.restore();
    }
